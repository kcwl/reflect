...
TEST(pod,complex_typpe)
{
	std::size_t size = reflect::tuple_size_v<leg>;

	EXPECT_TRUE(size == 4);

	std::size_t a_size = reflect::tuple_size_v<query_role_list_body_response>;

	EXPECT_TRUE(a_size == 1);

	std::size_t str_size = reflect::tuple_size_v<std::string>;

	EXPECT_TRUE(str_size == 1);
}